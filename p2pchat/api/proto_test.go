...
func TestJsonEncode(t *testing.T) {
	...
	event := P2PMessageEvent{
		MessageEvent: MessageEvent{
			Event:   Event{ID: "1", Time: now, Type: "message", DetailType: "private"},
			Message: Message{},
		},
	}
	...
	assert.Equal(fmt.Sprintf(`{"id":"1","time":"%v","type":"message","detail_type":"private","message":[]}`, now.Format(time.RFC3339Nano)), string(bytes))
}
...
func TestJsonDecode(t *testing.T) {
	...
	assert.Equal(event, P2PMessageEvent{
		MessageEvent: MessageEvent{
			Event:   Event{ID: "1", Type: "message", DetailType: "private"},
			Message: Message{},
		},
	})
